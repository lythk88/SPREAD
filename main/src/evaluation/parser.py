...
def parse_ground_truth(example: Dict[str, Any], data_name):
    # Special case for evaluate_sdad.py which passes a dict with only answer field and math
    if list(example.keys()) == ["problem", "answer", "solution"] and example["problem"] == "" and example["solution"] == "" and data_name in ["math"]:
        # This is the special case from evaluate_sdad.py, directly use the answer
        gt_cot = ""
        gt_ans = example["answer"]

        # Minimal processing to ensure we have a usable answer
        if gt_ans and isinstance(gt_ans, str):
            # Try to clean up the answer a bit
            gt_ans = gt_ans.strip()
            if data_name not in STRIP_EXCEPTIONS:
                gt_ans = strip_string(gt_ans, skip_unit=False)

            # For math problems, try to extract a number if we have one
            if data_name == "math" and not gt_ans:
                number_matches = re.findall(r'-?\d*\.?\d+', example["answer"])
                if number_matches:
                    gt_ans = number_matches[-1]
    ...
    # Final sanity check - if we still have an empty answer, try to get a number from the original answer
    if not gt_ans or gt_ans.strip() == '':
        try:
            # Look for any numbers in the original answer field, if present
            if "answer" in example and example["answer"]:
                number_matches = re.findall(r'-?\d*\.?\d+', str(example["answer"]))
                if number_matches:
                    gt_ans = number_matches[-1]
        except:
            pass
